...
impl TryInto<toml::Value> for EntryValue {
    ...
    fn try_into(self) -> Result<toml::Value, Self::Error> {
        match self {
            EntryValue::Entry(GobiFileEntry::Toml(val)) => Ok(val.data),
            EntryValue::Entry(_) => Err(GobiError {
                code: 1,
                msg: "Tried converting non TomlEntry value into a toml::Value".into(),
            }),
...
impl GobiFileTrait for TomlFile {
    ...
    fn save(&self) -> GobiResult<()> {
        ...
        match file.write(self.data.to_string().as_bytes()) {
            ...
            Err(..) => Err(GobiError {
                code: 1,
                msg: format!("Could not write to file '{}'", self.path.to_str().unwrap()),
            }),
        }
    }
}
